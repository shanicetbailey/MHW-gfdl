#to run in terminal paste: sbatch --job-name=lhflx --export=SCRIPT=1c2a_composite_lhflx.py submit_composite.sh
...
HF_VAR     = 'lhflx'  # derived: evap * -2.5e6

# ── Load evap ensemble and derive lhflx ──────────────────────────────────────
def preprocess_evap(ds):
    return ds[['evap']].sel(lat=slice(10, 31), lon=slice(262, 344), time=slice(TIME_START, TIME_END))

print("Opening evap ensemble...")
evap_ds = xr.open_mfdataset(
    [f'{BASE_PATH.format(e=e)}/evap_ens{e}_1991-2100.nc' for e in formatted_list],
    preprocess=preprocess_evap,
    concat_dim='ensemble',
    combine='nested',
    coords='minimal',
    compat='override',
...
evap_ds = evap_ds.assign_coords(ensemble=[int(e) for e in formatted_list])
...
evap_template = evap_ds['evap'].isel(ensemble=0, time=0).compute()
mask_template = mask_ds.event_mask.isel(ensemble=0, time=0).compute()
regridder = xe.Regridder(evap_template, mask_template, method='bilinear')
...
for i in range(30):
    ...
    evap        = evap_ds['evap'].isel(ensemble=i).compute()
    mask_member = mask_ds.event_mask.isel(ensemble=i).compute()

    # Derive lhflx from evap
    flux = evap * -2.5e6
    flux.name = HF_VAR
    ...
    del evap, flux, flux_ng, mask_member, mask_aligned, clim, anom, anom_mhw, anom_mhw_season, anom_nomhw, anom_nomhw_season # free memory
