#to run in terminal paste: sbatch --job-name=swflx --export=SCRIPT=1c2a_composite_swflx.py submit_composite.sh
...
HF_VAR     = 'swflx'  # derived: swdn_sfc - swup_sfc

# ── Load swdn_sfc and swup_sfc ensembles ─────────────────────────────────────
def preprocess_swdn(ds):
    return ds[['swdn_sfc']].sel(lat=slice(10, 31), lon=slice(262, 344), time=slice(TIME_START, TIME_END))

def preprocess_swup(ds):
    return ds[['swup_sfc']].sel(lat=slice(10, 31), lon=slice(262, 344), time=slice(TIME_START, TIME_END))

print("Opening swdn_sfc ensemble...")
swdn_ds = xr.open_mfdataset(
    [f'{BASE_PATH.format(e=e)}/swdn_sfc_ens{e}_1991-2100.nc' for e in formatted_list],
    preprocess=preprocess_swdn,
    concat_dim='ensemble',
    combine='nested',
    coords='minimal',
    compat='override',
...
swdn_ds = swdn_ds.assign_coords(ensemble=[int(e) for e in formatted_list])

print("Opening swup_sfc ensemble...")
swup_ds = xr.open_mfdataset(
    [f'{BASE_PATH.format(e=e)}/swup_sfc_ens{e}_1991-2100.nc' for e in formatted_list],
    preprocess=preprocess_swup,
    concat_dim='ensemble',
    combine='nested',
    coords='minimal',
    compat='override',
...
swup_ds = swup_ds.assign_coords(ensemble=[int(e) for e in formatted_list])
...
for e in formatted_list:
    ds = xr.open_mfdataset(
        MASK_PATH.format(e=e), decode_timedelta=True,
    ).rename_vars({'__xarray_dataarray_variable__': 'event_mask'})
...
flux_template = swdn_ds['swdn_sfc'].isel(ensemble=0, time=0).compute()
...
for i in range(30):
    ...
    swdn        = swdn_ds['swdn_sfc'].isel(ensemble=i).compute()
    swup        = swup_ds['swup_sfc'].isel(ensemble=i).compute()
    ...
    del swdn, swup, flux, flux_ng, mask_member, mask_aligned, clim, anom, anom_mhw, anom_mhw_season, anom_nomhw, anom_nomhw_season # free memory
